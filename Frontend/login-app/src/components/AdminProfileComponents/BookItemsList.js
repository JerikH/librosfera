  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        const bookResponse = await axios.get(`http://localhost:5000/api/v1/libros/${id}`, {
        ...
        const ejemplaresResponse = await axios.get(`http://localhost:5000/api/v1/ejemplares?idLibro=${id}`, {
          headers: { 'Authorization': `Bearer ${token}` }
        });
        ...
        if (ejemplaresResponse.data && ejemplaresResponse.data.status === 'success') {
          ...
          const tiendasPromises = tiendaIds.map(tiendaId => 
            axios.get(`http://localhost:5000/api/v1/tiendas/${tiendaId}`, {
              headers: { 'Authorization': `Bearer ${token}` }
            })
          );
